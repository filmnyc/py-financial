...
from applets.applet import go_ahead, currency, trans_header
...
def tedit_menu(selected, selected_transaction, page):
    ...
    while show == 'y':
        ...
        if edit_it.isdigit() == True and int(edit_it) >= 1 and int(edit_it) <= 5:
            return edit_it
        else:
            system('clear')
            trans_header(selected, selected_transaction, page)
            show = 'y'
            message = '"It must be a \'number\' between 1 and 5"'

def edit_one(selected, selected_transaction, page):
...
def edit_two(selected, selected_transaction, page):
    tran_0, tran_1, tran_3 = trans_header(selected, selected_transaction, page)
    ...
    if tran_yn == 'y':
        ...
        do_trans = go_ahead("If this is okay enter  ")
        if do_trans == 'y':
...
def edit_three(selected, selected_transaction, page):
    tran_0, tran_1, tran_3 = trans_header(selected, selected_transaction, page)
...
def edit_four(selected, selected_transaction, page):
    tran_0, tran_1, tran_3 = trans_header(selected, selected_transaction, page)
